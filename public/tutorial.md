...
### 基础用法
在元素上添加 `<!-- auto-animate -->` 注释，实现跨幻灯片的平滑过渡：
...
### 高级用法
使用 `data-id` 属性精确匹配特定元素，即使它们在不同幻灯片中有不同内容：
...
示例：`<!-- auto-animate: type=move, duration=800, easing=ease-out -->`
...
点击代码块右上角的“Run”按钮即可执行代码并查看输出结果。

### 插件市场
可通过插件市场安装各种功能插件，如图表生成、数据分析等。点击工具栏中的插件按钮访问插件市场。
